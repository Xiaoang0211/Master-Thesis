...
class GlobalContextAttention(torch.nn.Module):
    ...
    def forward(self, embedding):
        ...
        """
        easy_pooling = torch.sum(embedding, dim=1)
        easy_pooling = easy_pooling.unsqueeze(-1)
...
class PoolingWithAttention(torch.nn.Module):
    """
    SimGNN Attention Module to make a pass on graph.
    """
    def __init__(self, args):
        ...
        :param args: Arguments object.
        """
        super(PoolingWithAttention, self).__init__()
        self.args = args
        self.setup_weights()
        self.init_parameters()
...
class GeM(nn.Module):
    ...
    def gem(self, x, p=3, eps=1e-6):
        return (x.clamp(min=eps).pow(p).mean(dim=1).pow(1./p)).unsqueeze(-1)
...
class TensorNetworkModule(torch.nn.Module):
    ...
    def forward(self, embedding_1, embedding_2):
        ...
        :param embedding_1: Result of the 1st embedding after attention.    bxfx1 now: batch_size x num_pairs x feat_dim x 1
