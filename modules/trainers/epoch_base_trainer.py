...
class EpochBasedTrainer(BaseTrainer):
    ...
    def train_epoch(self):
        ...
        for iteration, data_dict_batched in enumerate(self.train_loader):

            self.inner_iteration = iteration + 1
            self.iteration += 1
            
            data_dict_batched = torch_util.to_cuda(data_dict_batched)
            self.before_train_step(self.epoch, self.inner_iteration, data_dict_batched)
            self.timer.add_prepare_time()

            # forward
            output_dict, result_dict = self.train_step(self.epoch, self.inner_iteration, data_dict_batched)
            # backward & optimization
            # if self.cfg.train.freeze_sgaligner:
            result_dict['loss'].backward(retain_graph=True)
            # result_dict['loss'].backward(retain_graph=True)
            self.after_backward(self.epoch, self.inner_iteration, data_dict_batched, output_dict, result_dict)
            self.check_gradients(self.epoch, self.inner_iteration, data_dict_batched, output_dict, result_dict)
            self.optimizer_step(self.inner_iteration)

            # after training
            self.timer.add_process_time()
            self.after_train_step(self.epoch, self.inner_iteration, data_dict_batched, output_dict, result_dict)
