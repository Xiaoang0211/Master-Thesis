...
from utilities.utils_node_match.timer import Timer
from utilities.utils_node_match.summary_board import SummaryBoard
...
class BaseTrainer(abc.ABC):
    ...
    def save_snapshot(self, filename):
        ...
        state_dict['optimizer_node'] = self.optimizer_node.state_dict()
        state_dict['optimizer_graph'] = self.optimizer_graph.state_dict()
    ...
    def load_snapshot(self, snapshot, fix_prefix=True):
        ...
        if 'optimizer' in state_dict and self.optimizer is not None:
            self.optimizer_node.load_state_dict(state_dict['optimizer_node'])
            self.optimizer_graph.load_state_dict(state_dict['optimizer_graph'])
    ...
    def register_model(self, model):
        ...
        if self.distributed:
            ...
            model = model.cuda()  # This will move the model to the default GPU
        self.model = model
        message = 'Model description:\n' + str(model)
        self.logger.info(message)
        return model

    def register_optimizer(self, optimizer_node, optimizer_graph):
        r"""Register optimizer. DDP is automatically used."""
        if self.distributed:
            for param_group in optimizer_node.param_groups:
                param_group['lr'] = param_group['lr'] * self.world_size
        self.optimizer_node = optimizer_node
        
        if self.distributed:
            for param_group in optimizer_graph.param_groups:
                param_group['lr'] = param_group['lr'] * self.world_size
        self.optimizer_graph = optimizer_graph
        
    def register_optimizer_PNV(self, optimizer_PNV):
        r"""Register optimizer. DDP is automatically used."""
        if self.distributed:
            for param_group in optimizer_PNV.param_groups:
                param_group['lr'] = param_group['lr'] * self.world_size
        self.optimizer_PNV = optimizer_PNV
    ...
    def get_lr(self):
        learning_rates = {}
        if self.cfg.train.freeze_sgaligner:
            lr_node = 0
        else:
            lr_node = self.optimizer_node.param_groups[0]['lr']
            
        lr_graph = self.optimizer_graph.param_groups[0]['lr']
        learning_rates['lr_node'] = lr_node
        learning_rates['lr_graph'] = lr_graph
