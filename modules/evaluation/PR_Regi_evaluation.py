...
from modules.evaluation.single_tester import SingleTester, SingleTester_simple
from modules.evaluation.registration_evaluator import RegistrationEvaluator
...
from modules.GeoTransformer.config import make_cfg as make_cfg_reg

class AlignerRetrievalTester(SingleTester_simple):
    def __init__(self, cfg, parser):
        ...
        self.visualise_registration = False
    ...
    def get_topk_idx(self, pred_db, k=1):
        top_k_idx = np.argsort(pred_db)[-k:]
        return top_k_idx
    
    def eval_step(self, iteration, data_dict, output_dict, rank_lists):
        ...
        for batch_idx in range(real_batch_size):
            ...
            if e1i_idxs.shape[0] != 0 and e2i_idxs.shape[0] != 0:
                ...
                if self.run_reg:
                    ...
                    # node_corrs = alignment.get_node_corrs_objects_ids_directly(node_corrs, src_obj_ids, ref_obj_ids)
                    ...
                    if scan_id in self.reference_ids:
                        ...
                        label_file_name = "labels.instances.annotated.v2.align.ply"
                    ...
                    raw_points_dir = osp.join(self.test_dataset.data_root_dir, 'scenes', scan_id, label_file_name)
                    raw_points = scan3r.read_ply_points(osp.join(self.test_dataset.data_root_dir, 'scenes', scan_id, label_file_name))
    ...
    def eval_step_registration(self, rank_list, src_objects_count, all_objects_ids, src_scan_id, ref_scan_id, pcl_center):
        node_corrs = alignment.compute_node_corrs(rank_list, src_objects_count, self.reg_k)
        node_corrs = alignment.get_node_corrs_objects_ids_directly(node_corrs, all_objects_ids)
        
        # Load subscene points
        scan_id = src_scan_id[:src_scan_id.index('_')] # scan id src
        scan_id_ref = ref_scan_id[:ref_scan_id.index('_')]
        ...
        src_points, src_plydata = scan3r.load_plydata_npy(osp.join(self.test_dataset.scans_scenes_dir, src_scan_id, 'data.npy'), obj_ids=None, return_ply_data=True)
        ref_points, ref_plydata = scan3r.load_plydata_npy(osp.join(self.test_dataset.scans_scenes_dir, ref_scan_id, 'data.npy'), obj_ids=None, return_ply_data=True)
        src_obj_ids = np.stack([src_plydata['objectId']]).squeeze(0)
        ref_obj_ids = np.stack([ref_plydata['objectId']]).squeeze(0)
        ...
        if scan_id == scan_id_ref:
            _, gt_src_corr_idxs = point_cloud.compute_pcl_overlap(reg_data_dict['src_points'], reg_data_dict['ref_points'] )
            _, gt_ref_corr_idxs = point_cloud.compute_pcl_overlap(reg_data_dict['ref_points'] , reg_data_dict['src_points'])
        else:
            #  _, gt_src_corr_idxs = point_cloud.compute_pcl_overlap(reg_data_dict['src_points'], 
            #                                                         reg_data_dict['ref_points'],
            #                                                         threshold=0.05)
            # _, gt_ref_corr_idxs = point_cloud.compute_pcl_overlap(reg_data_dict['ref_points'], 
            #                                                         reg_data_dict['src_points'],
            #                                                         threshold=0.05)
            _, gt_src_corr_idxs, gt_ref_corr_idxs = point_cloud.compute_pcl_overlap4rescans(reg_data_dict['src_points'], 
                                                                                            reg_data_dict['ref_points'],
                                                                                            src_obj_ids,
                                                                                            ref_obj_ids)
            # _, gt_ref_corr_idxs = point_cloud.compute_pcl_overlap4rescans(reg_data_dict['ref_points'], 
            #                                                               reg_data_dict['src_points'],
            #                                                               ref_obj_ids,
            #                                                               src_obj_ids)
        ...
        return {'normal_registration_metrics' : self.normal_registration_metrics_meter, 'aligner_registration_metrics' : self.aligner_registration_metrics_meter}
