...
from utilities.utils_node_match import common, point_cloud, scan3r, open3d, visualisation
from utilities.utils_node_match.logger import Logger
...
class SubGenScan3R(Dataset):
    ...
    def __init__(self, cfg, split='train'):
        ...
        self.dynm_anchors_included = cfg.data.dynm_anchors_included
        ...
        self.reference_ids = scan3r.get_scan_ids(self.file_dir, 'ref')
        ...
        # dir to rescan to reference json
        self.scan2rescan_meta = common.load_json(osp.join(self.file_dir, 'anchors_rescans_to_refscans_{}.json'.format(self.split)))

        self.num_subscans_per_scan = cfg.preprocess.subscenes_per_scene
        self.subscene_rels = {'scans' : []}
        self.subscene_objs = {'scans' : []}
        self.subscans_only = True
        ...
        self.label_file_name = None if not self.predicted else 'inseg_filtered.ply'
    ...
    def pair_exist(self, data, src_value, ref_value):
        # Convert the src and ref from each dictionary into a frozenset
        pairs_as_frozensets = {frozenset([d['src'], d['ref']]) for d in data}
        return frozenset([src_value, ref_value]) in pairs_as_frozensets
    ...
    def calculate_overlap(self):
        ...
        for scan_id in tqdm(self.scan_ids):
            # subscans with the same scan id
            subscan_ids = [subscan for subscan in all_subscan_ids if subscan.startswith(scan_id)]

            # # print(scan_id)
            # if self.subscans_only:
            #     subscan_ids.remove(scan_id)
                
            # registration pair among the subscans
            ...
            for subscan_pair in subscan_idx_pairs:
                ...
                if overlap_ratio > 0.1 and overlap_ratio <= 1:
            ...
            # read the meta data for rescan-refscan mapping
            src_list = [src_scan_id['src'] for src_scan_id in self.scan2rescan_meta if src_scan_id['ref'] == scan_id]
            ref_list = [src_scan_id['ref'] for src_scan_id in self.scan2rescan_meta if src_scan_id['src'] == scan_id]
            rescan_list = src_list + ref_list

            rescan_subscans = []
            if len(rescan_list) != 0:
                for rescan_id in rescan_list:
                    subscan_ids_rescans = [subscan for subscan in all_subscan_ids if subscan.startswith(rescan_id)]
                    rescan_subscans += subscan_ids_rescans
            
            subscan_subrescan_pairs = list(itertools.product(subscan_ids, rescan_subscans))
            
            for subscan_subrescan_pair in subscan_subrescan_pairs:
                src_idx = subscan_subrescan_pair[0]
                ref_idx = subscan_subrescan_pair[1]

                _, src_ply_data = scan3r.load_plydata_npy(osp.join(self.scene_out_dir, src_idx, 'data.npy'), obj_ids=None, return_ply_data=True)
                _, ref_ply_data = scan3r.load_plydata_npy(osp.join(self.scene_out_dir, ref_idx, 'data.npy'), obj_ids=None, return_ply_data=True)

                src_points = np.stack([src_ply_data['x'], src_ply_data['y'], src_ply_data['z']]).transpose((1, 0))
                ref_points = np.stack([ref_ply_data['x'], ref_ply_data['y'], ref_ply_data['z']]).transpose((1, 0))
                src_obj_ids = np.unique(np.stack([src_ply_data['objectId']]).squeeze(0))
                ref_obj_ids = np.unique(np.stack([ref_ply_data['objectId']]).squeeze(0))

                
                
                # overlap_ratio, common_pts_idx_src = point_cloud.compute_pcl_overlap4rescans(src_points, ref_points, src_obj_ids, ref_obj_ids, threshold=0.05)
                overlap_ratio, common_pts_idx_src = point_cloud.compute_pcl_overlap(src_points, ref_points, threshold=0.05)
                
                if overlap_ratio == 0:
                    continue
                
                overlapped_points = src_points[common_pts_idx_src]
                # if overlap_ratio <= 0.0 and overlap_ratio > 0.05:
                fig = plt.figure(figsize=(10, 10))
                ax = fig.add_subplot(111, projection='3d')
                ax.scatter(src_points[:, 0], src_points[:, 1], src_points[:, 2], s=0.5, c='g', label='Source Point Cloud', alpha=0.2)
                ax.scatter(ref_points[:, 0], ref_points[:, 1], ref_points[:, 2], s=0.5, c='b', label='Reference Point Cloud', alpha=0.2)
                ...
                # since we skipped the case where there is no overlap the if condition is no more necessary
                if overlap_ratio > 0.05 and overlap_ratio <= 1:
                    if self.dynm_anchors_included == True:
                        anchor_obj_ids = np.intersect1d(src_obj_ids, ref_obj_ids)
                    else:
                        anchor_obj_ids = np.unique(src_ply_data['objectId'][common_pts_idx_src])
                    data = {'src' : src_idx, 'ref' : ref_idx, 
                            'overlap' : overlap_ratio, 'anchorIds' : anchor_obj_ids.tolist()}
                    if not self.pair_exist(overlap_data, data["src"], data["ref"]):
    ...
    def __getitem__(self, data):
        ...
        if scan_id in self.reference_ids:
            self.label_file_name = "labels.instances.annotated.v2.ply"
        else:
            self.label_file_name = "labels.instances.annotated.v2.align.ply"
        ...
        # Load scene pcl
        assert self.label_file_name != None
