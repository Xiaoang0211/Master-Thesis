...
def get_node_corrs_objects_ids_directly(node_corrs, objects_ids):
    node_corrs_objects_ids = []

    for node_corr in node_corrs:
        node_corrs_objects_ids.append((objects_ids[node_corr[0]], objects_ids[node_corr[1]]))
    return node_corrs_objects_ids
...
def compute_recall_pr(pred_db, gt_db, k, scores):
    ...
    num_ones = np.sum(gt_db)
    
    if min(top_k_idx) <= (num_ones - 1):
        scores.append(1)
    else:
        scores.append(0)
...
#     # Assuming pred_db and gt_db are already PyTorch tensors
#     # If not, convert them using torch.tensor(pred_db) and torch.tensor(gt_db)

#     # Get top k indices based on pred_db values
#     top_k_idx = torch.argsort(pred_db, descending=True)[:k]
#     num_ones = torch.sum(gt_db)

#     # Check if any of the top k indices are within the range of ones in gt_db
#     if torch.any(top_k_idx < num_ones):
#         scores.append(1)
#     else:
#         scores.append(0)
